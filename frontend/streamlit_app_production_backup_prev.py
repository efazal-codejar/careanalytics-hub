...
def page_ai_analytics():
    ...
    **Free AI Analytics** powered by your database - Ask questions about care gaps, HEDIS compliance, 
    ...
    questions_pressed = [False] * 6
    
    with col1:
        if st.button("📊 What are our biggest care gaps?", key="q1"):
            questions_pressed[0] = True
        
        if st.button("👨‍⚕️ Which providers need support?", key="q2"):
            questions_pressed[1] = True
    
    with col2:
        if st.button("📈 What's our HEDIS status?", key="q3"):
            questions_pressed[2] = True
        
        if st.button("📈 How are we trending vs targets?", key="q4"):
            questions_pressed[3] = True
    
    with col3:
        if st.button("😊 What's affecting satisfaction?", key="q5"):
            questions_pressed[4] = True
        
        if st.button("⚠️ What about our high-risk patients?", key="q6"):
            questions_pressed[5] = True
    ...
    with st.expander("ℹ️ How This Works"):
        ...
        **Example Questions You Can Ask:**
        ...
        - "What's our HEDIS compliance status?" → Breaks down measure performance
        - "What about high-risk patients?" → Risk stratification & intervention strategies
        - "How can we improve satisfaction?" → Actionable recommendations
        
        **Optional: Setup Ollama for Better Responses**
        
        For more natural AI responses, install Ollama (free, local):
        
        1. Download from https://ollama.ai
        2. Run: `ollama pull mistral` (or another model)
        3. This app will automatically detect and use it
        4. Still 100% free - no cloud costs!
        """)
